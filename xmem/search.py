...
def search_cards(query: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    for card in cards:
        ...
        strong_parts = [card.get("card_id", ""), card.get("title", ""), project.get("name", ""), project.get("root", "")]
        ...
        alias_text = "\n".join(str(x).lower() for x in alias_parts + [card.get("title", "")])
        ...
        for variant in variants:
            ...
            if not loose_variant:
                continue
            if loose_variant in alias_norm:
                score += 12.0
    ...
    append_jsonl(home_dir() / "gain.jsonl", {
        "ts": utc_now(), "event": "context.hit" if result else "context.miss", "query": query,
        "matches": len(result), "estimated_tokens_saved": len(result) * 1200,
    })
    return result
